...
def resolve_tagged_file_parts(project_id: str, payload: Dict[str, Any]) -> FileTagResolution:
    ...
    absolute_files: List[str] = []
    aggregated_text_pieces: List[str] = []
    preamble_added = False

    def append_segment(segment: Dict[str, Any]) -> None:
        text = segment.get("text", "")
        if text:
            aggregated_text_pieces.append(text)
        text_segments.append(segment)

    def ensure_preamble() -> None:
        nonlocal preamble_added
        if not preamble_added:
            append_segment({"type": "preamble", "text": "\n--- Content from referenced files ---"})
            preamble_added = True
        ...
        if os.path.isdir(absolute_path):
            ...
            for child_path in files_in_dir:
                ...
                if _is_text_file(child_path, mime_type):
                    ...
                    ensure_preamble()
                    intro_text = f"\nContent from @{rel_child}"
                    if truncated:
                        intro_text += f" (truncated to {MAX_TEXT_CHARACTERS} characters)"
                    intro_text += ":\n"
                    append_segment({
                        "type": "file_intro",
    ...
    if preamble_added:
        append_segment({"type": "epilogue", "text": "\n--- End of content ---"})

    return FileTagResolution(
        parts=parts,
        text_segments=text_segments,
        aggregated_text="".join(aggregated_text_pieces),
